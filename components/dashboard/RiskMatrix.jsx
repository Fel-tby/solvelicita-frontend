...
export default function RiskMatrix({ municipios, onSelect }) {
  ...
  if (!data.length) {
    return (
      <div className="dash-v3-section" style={{ minHeight: 300, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        <p style={{ color: 'var(--text-lo)', fontSize: '0.85rem' }}>Dados insuficientes para a Matriz de Risco.</p>
      </div>
    )
  }
  ...
  // Quadrant labels
  const quadrants = [
    { label: 'Due diligence obrigatória', x: PADDING.left + 6, y: PADDING.top + 14, color: '#ef4444' },
    { label: 'Prioridade comercial', x: midX + 6, y: PADDING.top + 14, color: '#16a34a' },
    { label: 'Baixa prioridade', x: PADDING.left + 6, y: HEIGHT - PADDING.bottom - 8, color: '#94a3b8' },
    { label: 'Prospectar c/ monitoramento', x: midX + 6, y: HEIGHT - PADDING.bottom - 8, color: '#f59e0b' },
  ]

  // Legend items
  const legend = [
    { label: 'Risco Baixo', color: CORES_RISCO_HEX.baixo },
    { label: 'Risco Médio', color: CORES_RISCO_HEX.medio },
    { label: 'Risco Alto', color: CORES_RISCO_HEX.alto },
  ...
  return (
    <div className="dash-v3-section">
      <div className="dash-v3-section-header" style={{ marginBottom: 0 }}>
        <h2 className="dash-v3-section-title">3. Matriz Risco × Oportunidade</h2>
        {/* Inline legend */}
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: '0.7rem', color: 'var(--text-lo)', marginLeft: 8 }}>
            <span style={{ fontSize: '0.6rem' }}>◯</span> tamanho = nº licitações
          </div>
        </div>
      </div>
      ...
      <div className="dash-v3-matrix-container">
        <svg width="100%" height="100%" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} preserveAspectRatio="xMidYMid meet">
          ...
          <text x={PADDING.left + innerW / 2} y={HEIGHT - 4} fill="var(--text-mid)" fontSize="10" fontWeight="600" textAnchor="middle" fontFamily="var(--sans)">Score de confiabilidade →</text>
          ...
          <text x={15} y={PADDING.top + innerH / 2} fill="var(--text-mid)" fontSize="10" fontWeight="600" textAnchor="middle" fontFamily="var(--sans)" transform={`rotate(-90 15 ${PADDING.top + innerH / 2})`}>Valor homologado →</text>
              ...
              <foreignObject x={tx} y={ty} width={tw} height={th} style={{ pointerEvents: 'none' }}>
                <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '6px 10px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', fontFamily: 'var(--sans)' }}>
                  ...
                  <div style={{ fontSize: '0.7rem', color: 'var(--text-mid)', display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '1px 8px' }}>
                    ...
                    <span>Valor:</span><span style={{ fontWeight: 600 }}>{fmtValorHomologado(hoveredNode.valorNum)}</span>
                    <span>Licit.:</span><span style={{ fontWeight: 600 }}>{hoveredNode.licitacoesNum}</span>
                  </div>
                </div>
              </foreignObject>
            )
