...
export function calcularRecomendacao(municipio) {
  ...
  if (pendenciasGraves >= 2) return { label: 'Due diligence', cor: 'var(--risk-alto)' }
  
  if (Number.isNaN(score)) return { label: '-', cor: 'var(--text-lo)' }

  if (score >= 80) {
    if (pendenciasGraves === 0) return { label: 'Priorizar prospecção', cor: 'var(--risk-baixo)' }
    return { label: 'Prospectar com monitoramento', cor: 'var(--risk-medio)' }
  }
  
  if (score >= 60) {
    if (pendenciasGraves === 0) return { label: 'Prospectar com monitoramento', cor: 'var(--risk-medio)' }
    return { label: 'Due diligence', cor: 'var(--risk-alto)' }
  }
  
  if (score >= 40) return { label: 'Due diligence', cor: 'var(--risk-alto)' }
  
  return { label: 'Evitar / mitigar', cor: 'var(--risk-critico)' }
}
