...
const MUNICIPIOS_LANDING_SELECT_FIELDS = ['uf', 'cod_ibge', 'score', 'classificacao']
const MUNICIPIOS_LANDING_SELECT = MUNICIPIOS_LANDING_SELECT_FIELDS.join(',')
...
async function fetchMunicipiosPaginated(selectFields) {
  const rows = []
  let from = 0
...
export async function fetchMunicipiosLandingSummary() {
  return fetchMunicipiosPaginated(MUNICIPIOS_LANDING_SELECT)
}
...
export function buildStateDashboardSummaries(rows) {
  const grouped = new Map()
  ...
  rows.forEach((row) => {
    ...
    bucket.scoreTotal += Number.isFinite(numericScore) ? numericScore : 0
    bucket.scoreCount += Number.isFinite(numericScore) ? 1 : 0
    grouped.set(uf, bucket)
  })

  return buildStateSummaries(rows).map((summary) => {
    const stats = grouped.get(summary.uf)
    const scoreMedio = stats?.scoreCount ? stats.scoreTotal / stats.scoreCount : null

    return {
      ...summary,
      scoreMedio,
      altoCritico: (summary.alto || 0) + (summary.critico || 0),
      coverageRatio:
        summary.municipios && summary.total
          ? Math.min(summary.total / summary.municipios, 1)
          : 0,
    }
  })
}
